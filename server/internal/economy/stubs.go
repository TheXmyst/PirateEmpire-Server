...
// IsPositionClear checks if a position is clear of islands (collision check)
func IsPositionClear(x, y float64, allIslands []domain.Island) bool {
	const minDistance = 100.0 // Minimum distance from any island

	// Check distance from all islands
	for _, island := range allIslands {
		dx := float64(island.X) - x
		dy := float64(island.Y) - y
