...
var (
	chatRateLimiter = struct {
		sync.Mutex
		last map[uuid.UUID]time.Time
	}{last: make(map[uuid.UUID]time.Time)}
	chatRateWindow = 2 * time.Second
	bannedWords    = []string{"fuck", "shit", "merde", "putain", "bitch", "pute"}
)

func resolvePlayerSeaID(db *gorm.DB, playerID uuid.UUID) (uuid.UUID, error) {
	var island domain.Island
	if err := db.Select("sea_id").Where("player_id = ?", playerID).First(&island).Error; err != nil {
		return uuid.Nil, err
	}
	if island.SeaID == uuid.Nil {
		return uuid.Nil, fmt.Errorf("player has no sea assigned")
	}
	return island.SeaID, nil
}
...
// SendChat allows authenticated players to post a chat message.
func SendChat(c echo.Context) error {
	...
	if req.Message == "" {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Message is empty"})
	}
	if len(req.Message) > 256 {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Message too long (max 256)"})
	}

	if containsBannedWord(req.Message) {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Message refusé"})
	}

	now := time.Now().UTC()
	chatRateLimiter.Lock()
	if last, ok := chatRateLimiter.last[player.ID]; ok {
		if now.Sub(last) < chatRateWindow {
			wait := chatRateWindow - now.Sub(last)
			chatRateLimiter.Unlock()
			return c.JSON(http.StatusTooManyRequests, map[string]string{"error": fmt.Sprintf("Trop de messages, attends %ds", int(wait.Seconds())+1)})
		}
	}
	chatRateLimiter.last[player.ID] = now
	chatRateLimiter.Unlock()

	db := repository.GetDB()
	seaID, err := resolvePlayerSeaID(db, player.ID)
	if err != nil {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Aucune île trouvée pour déterminer la mer"})
	}
	...
	if err := db.Create(&msg).Error; err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Impossible d'enregistrer le message"})
	}

	return c.JSON(http.StatusOK, map[string]string{"status": "ok"})
}

// GetChat returns messages newer than optional ?since=RFC3339
func GetChat(c echo.Context) error {
	...
	if err := query.Order("created_at desc").Limit(limit + 1).Find(&msgs).Error; err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Impossible de charger le chat"})
	}
	...
	if hasMore && len(msgs) > 0 {
		nextCursor = msgs[0].CreatedAt.Format(time.RFC3339)
	}
...
func GetStatus(c echo.Context) error {
	...
	// DEBUG: Check if UnlockedTechs are loaded
	if os.Getenv("DEBUG_TECHS") != "" {
		fmt.Printf("[TECH_DEBUG] GetStatus: Player ID=%s, UnlockedTechs count=%d, UnlockedTechsJSON len=%d\n",
			player.ID, len(player.UnlockedTechs), len(player.UnlockedTechsJSON))
		if len(player.UnlockedTechs) > 0 {
			fmt.Printf("[TECH_DEBUG] Techs: %v\n", player.UnlockedTechs)
		}
	}
	...
	if len(player.Islands) > 0 {
		...
		// Check for finished militia recruitment
		for i := range player.Islands {
			// Pass reference to element in slice
			if CheckIslandRecruitment(db, &player.Islands[i]) {
				// If changed, the slices might be stale if we append/modify structure,
				// but here we just updated fields in place.
				// GORM save was called inside helper.
			}
		}
		...
		if os.Getenv("DEBUG") != "" || os.Getenv("CAPTAIN_DEBUG") != "" {
			for _, fleet := range island.Fleets {
				for _, ship := range fleet.Ships {
					if ship.MilitiaWarriors > 0 || ship.MilitiaArchers > 0 || ship.MilitiaGunners > 0 {
						fmt.Printf("[STATUS] ship_id=%s crew: warriors=%d archers=%d gunners=%d\n",
							ship.ID, ship.MilitiaWarriors, ship.MilitiaArchers, ship.MilitiaGunners)
					}
				}
			}
		}
...
func StartShipConstruction(c echo.Context) error {
	...
	if config.RequiredTechID != "" {
		hasTech := false
		// Hydrate unlocked techs from authenticated player (most up-to-date)
		unlocked := player.UnlockedTechs
		if len(unlocked) == 0 && len(player.UnlockedTechsJSON) > 0 {
			_ = json.Unmarshal(player.UnlockedTechsJSON, &unlocked)
		}
		// Fallback to island preload player if still empty
		if len(unlocked) == 0 {
			unlocked = island.Player.UnlockedTechs
			if len(unlocked) == 0 && len(island.Player.UnlockedTechsJSON) > 0 {
		...
		if os.Getenv("DEBUG_TECHS") != "" {
			fmt.Printf("[TECH_DEBUG] Build ship %s requires %s | unlocked=%d (player=%d island=%d json_player=%d json_island=%d)\n",
				req.ShipType, config.RequiredTechID,
				len(unlocked), len(player.UnlockedTechs), len(island.Player.UnlockedTechs),
				len(player.UnlockedTechsJSON), len(island.Player.UnlockedTechsJSON))
		}
		if !hasTech {
			tx.Rollback()
	...
	bonuses := economy.CalculateTechBonuses(techs)
	buildTimeSec := economy.CalculateShipBuildTime(req.ShipType, bonuses.TechModifiers)
...
func AddShipToFleet(c echo.Context) error {
	...
	// 3. Check Capacity
	maxShips := economy.GetMaxShipsPerFleet(economy.ComputeTechModifiers(player.UnlockedTechs))
	...
	// 5. Update Ship
	fmt.Printf("[FLEET] AddShipToFleet: Persisting assignment... ship_id=%s -> fleet_id=%s\n", ship.ID, fleet.ID)
	...
	fmt.Printf("[FLEET] AddShipToFleet: Success! Ship %s (%s) added to fleet %s (%s)\n",
		ship.ID, ship.Name, fleet.ID, fleet.Name)
...
func DevGrantCaptain(c echo.Context) error {
	...
	return c.JSON(http.StatusOK, captain.ToDTO())
}
...
func AssignCaptain(c echo.Context) error {
	...
	// Convert to DTOs for consistent JSON serialization (string UUIDs)
	assignmentDTO := dto.ShipAssignmentDTO{
		Captain: captain.ToDTO(),
		Ship:    *ship.ToDTO(),
	}
...
func SummonCaptain(c echo.Context) error {
	...
	// Prepare results array (using DTO for consistent serialization)
	results := make([]dto.SummonResultDTO, 0, req.Count)
	...
	for i := 0; i < req.Count; i++ {
		...
		// Convert captain to DTO if not nil (i.e., not a duplicate)
		var captainDTO *dto.CaptainDTO
		if captain != nil {
			capDTO := captain.ToDTO()
			captainDTO = &capDTO
		}

		results = append(results, dto.SummonResultDTO{
...
func UpgradeCaptainStars(c echo.Context) error {
	...
	// Convert to DTO for consistent JSON serialization (string UUIDs)
	captainDTO := captain.ToDTO()
...
func DevSimulateEngagement(c echo.Context) error {
	...
	if flagshipA != nil {
		...
		result.Applied = append([]string{fmt.Sprintf("FleetA flagship selection: explicit=%v ship_id=%s type=%s reason=%s crew_dominant=%s (warriors=%d archers=%d gunners=%d)",
			explicitA, flagshipA.ID, flagshipA.Type, reasonA, dominantCrewA, flagshipA.MilitiaWarriors, flagshipA.MilitiaArchers, flagshipA.MilitiaGunners)}, result.Applied...)
	} else {
	...
	if flagshipB != nil {
		...
		result.Applied = append([]string{fmt.Sprintf("FleetB flagship selection: explicit=%v ship_id=%s type=%s reason=%s crew_dominant=%s (warriors=%d archers=%d gunners=%d)",
			explicitB, flagshipB.ID, flagshipB.Type, reasonB, dominantCrewB, flagshipB.MilitiaWarriors, flagshipB.MilitiaArchers, flagshipB.MilitiaGunners)}, result.Applied...)
	} else {
...
// DevSetShipMilitiaRequest is the request for the /dev/set-ship-militia endpoint
type DevSetShipMilitiaRequest struct {
...
// DevSetShipMilitiaResponse is the response for the /dev/set-ship-militia endpoint
type DevSetShipMilitiaResponse struct {
...
// DevSetShipMilitia sets the militia composition for a ship (dev only, for testing)
func DevSetShipMilitia(c echo.Context) error {
	// Get authenticated player from context
	player := auth.GetAuthenticatedPlayer(c)
	if err := checkDevAdmin(player); err != nil {
		return c.JSON(http.StatusForbidden, map[string]string{"error": "Accès refusé: admin uniquement"})
	...
	// Parse request
	req := new(DevSetShipMilitiaRequest)
	if err := c.Bind(req); err != nil {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": fmt.Sprintf("Requête invalide: %v", err)})
	}
	...
	// Validate crew counts (must be >= 0 and <= max)
	const MaxCrewPerShip = 10000
	if req.Warriors < 0 || req.Warriors > MaxCrewPerShip {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": fmt.Sprintf("warriors doit être entre 0 et %d", MaxCrewPerShip)})
	}
	if req.Archers < 0 || req.Archers > MaxCrewPerShip {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": fmt.Sprintf("archers doit être entre 0 et %d", MaxCrewPerShip)})
	}
	if req.Gunners < 0 || req.Gunners > MaxCrewPerShip {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": fmt.Sprintf("gunners doit être entre 0 et %d", MaxCrewPerShip)})
	}
	...
	// Update crew counts
	ship.MilitiaWarriors = req.Warriors
	ship.MilitiaArchers = req.Archers
	ship.MilitiaGunners = req.Gunners
	...
	fmt.Printf("[DEV] SetShipMilitia: ship_id=%s player_id=%s warriors=%d archers=%d gunners=%d\n",
		shipID, playerID, req.Warriors, req.Archers, req.Gunners)

	return c.JSON(http.StatusOK, DevSetShipMilitiaResponse{
		ShipID:   shipID.String(),
		Warriors: req.Warriors,
		Archers:  req.Archers,
		Gunners:  req.Gunners,
		Message:  "Milice mise à jour",
	})
}

// ==================== SOCIAL SYSTEM ====================

// SearchFriendsRequest for friend search
type SearchFriendsRequest struct {
	Query string `json:"query"`
}

// PlayerSummary for friend/leaderboard display
...
// AddFriendRequest to add a friend
type AddFriendRequest struct {
	FriendID uuid.UUID `json:"friend_id"`
}

// RemoveFriendRequest to remove a friend
type RemoveFriendRequest struct {
	FriendID uuid.UUID `json:"friend_id"`
}

// CreateGuildRequest to create a guild
type CreateGuildRequest struct {
	Name string `json:"name"`
}

// GuildSummary for guild info
...
// LeaderboardEntry for top players/guilds
type LeaderboardEntry struct {
	Rank       int       `json:"rank"`
	ID         uuid.UUID `json:"id"`
	Name       string    `json:"name"`
	Reputation int       `json:"reputation"`
	Members    int       `json:"members,omitempty"`  // For guilds
	Username   string    `json:"username,omitempty"` // For players
}

// SearchFriends endpoint to search for players
func SearchFriends(c echo.Context) error {
	player := auth.GetAuthenticatedPlayer(c)
	if player == nil {
		return c.JSON(http.StatusUnauthorized, map[string]string{"error": "Authentication required"})
	}
	...
	if err := db.Where("username LIKE ?", "%"+query+"%").Limit(50).Find(&players).Error; err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Search failed"})
	}

	result := make([]PlayerSummary, 0, len(players))
	for _, p := range players {
		if p.ID != playerID { // Don't show yourself
...
// AddFriend endpoint to add a friend
func AddFriend(c echo.Context) error {
	...
	if req.FriendID == playerID {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Cannot friend yourself"})
	}
	...
	if err := db.Model(&domain.Player{}).Where("id = ?", req.FriendID).Count(&friendCount).Error; err != nil {
	...
	if err := db.Where("player_id = ? AND friend_id = ?", playerID, req.FriendID).First(&existingFriend).Error; err == nil {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Already friends"})
	}
	...
	if err := db.Create(&friend).Error; err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to add friend"})
	}

	return c.JSON(http.StatusOK, map[string]string{"message": "Friend added"})
}

// RemoveFriend endpoint to remove a friend
func RemoveFriend(c echo.Context) error {
	...
	req := new(RemoveFriendRequest)
	if err := c.Bind(req); err != nil {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Invalid request"})
	}

	db := repository.GetDB()
	if err := db.Where("player_id = ? AND friend_id = ?", playerID, req.FriendID).Delete(&domain.Friend{}).Error; err != nil {
...
// ListFriends endpoint to get friends list
func ListFriends(c echo.Context) error {
	...
	if err := db.Where("player_id = ?", playerID).Find(&friends).Error; err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to load friends"})
	}

	result := make([]PlayerSummary, 0, len(friends))
	for _, f := range friends {
		var player domain.Player
		if err := db.Select("id, username, guild_id").First(&player, "id = ?", f.FriendID).Error; err == nil {
			result = append(result, PlayerSummary{
				ID:       player.ID,
...
// CreateGuild endpoint to create a guild
func CreateGuild(c echo.Context) error {
	...
	if name == "" {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Guild name is required"})
	}
	if len(name) > 50 {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Guild name too long"})
	}

	db := repository.GetDB()

	// Check if player already in a guild
	var playerData domain.Player
	if err := db.Select("id, guild_id, guild_ticket").First(&playerData, "id = ?", playerID).Error; err != nil {
		return c.JSON(http.StatusNotFound, map[string]string{"error": "Player not found"})
	}
	if playerData.GuildID != nil {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Already in a guild"})
	}
	if playerData.GuildTicket < 1 {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "No guild tickets available"})
	}

	// Check if guild name exists
	var existingGuild domain.Guild
	if err := db.Where("name = ?", name).First(&existingGuild).Error; err == nil {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Guild name already taken"})
	}
	...
	if err := db.Create(&guild).Error; err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to create guild"})
	}
	...
	if err := db.Create(&member).Error; err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to add founder"})
	}

	// Deduct ticket and update player's guild_id
	if err := db.Model(&domain.Player{}).Where("id = ?", playerID).Updates(map[string]interface{}{
		"guild_id":     guild.ID,
		"guild_ticket": gorm.Expr("guild_ticket - 1"),
	}).Error; err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to update player"})
	}

	return c.JSON(http.StatusOK, GuildSummary{
		ID:          guild.ID,
		Name:        guild.Name,
		OwnerID:     guild.OwnerID,
		Members:     []PlayerSummary{{ID: playerID, Username: player.Username}},
		MemberCount: 1,
		CreatedAt:   guild.CreatedAt,
	})
}

// GetGuild endpoint to get guild info
func GetGuild(c echo.Context) error {
	...
	// Get player's guild
	var playerData domain.Player
	if err := db.Select("id, guild_id").First(&playerData, "id = ?", playerID).Error; err != nil {
		return c.JSON(http.StatusNotFound, map[string]string{"error": "Player not found"})
	}
	if playerData.GuildID == nil {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "Not in a guild"})
	}

	// Get guild
	var guild domain.Guild
	if err := db.First(&guild, "id = ?", playerData.GuildID).Error; err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Guild not found"})
	}

	// Get members
	var members []domain.GuildMember
	db.Where("guild_id = ?", guild.ID).Find(&members)

	memberSummaries := make([]PlayerSummary, 0, len(members))
	for _, m := range members {
		var p domain.Player
		if err := db.Select("id, username").First(&p, "id = ?", m.PlayerID).Error; err == nil {
			memberSummaries = append(memberSummaries, PlayerSummary{
				ID:       p.ID,
				Username: p.Username,
			})
		}
	...
	return c.JSON(http.StatusOK, GuildSummary{
		...
		MemberCount: len(memberSummaries),
		CreatedAt:   guild.CreatedAt,
	})
}

// LeaveGuild endpoint to leave guild
func LeaveGuild(c echo.Context) error {
	...
	if err := db.Where("guild_id = ? AND player_id = ?", guildID, playerID).Delete(&domain.GuildMember{}).Error; err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to leave guild"})
	}

	// Update player guild_id to null
	if err := db.Model(&domain.Player{}).Where("id = ?", playerID).Update("guild_id", nil).Error; err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to update player"})
	}

	// If owner, try to disband guild (simplest approach: just leave it empty)
	// For now, we'll just remove the player, guild stays

	return c.JSON(http.StatusOK, map[string]string{"message": "Left guild"})
}

// GetLeaderboardPlayers endpoint for player rankings
func GetLeaderboardPlayers(c echo.Context) error {
	db := repository.GetDB()

	var players []domain.Player
	if err := db.Select("id, username, reputation").Find(&players).Error; err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to load leaderboard"})
	}

	// Use reputation field from player
	type PlayerReputation struct {
		ID         uuid.UUID
		Username   string
		Reputation int
	}
	playerReps := make([]PlayerReputation, 0, len(players))

	for _, p := range players {
		playerReps = append(playerReps, PlayerReputation{
			ID:         p.ID,
			Username:   p.Username,
	...
	sort.Slice(playerReps, func(i, j int) bool {
		return playerReps[i].Reputation > playerReps[j].Reputation
	})

	// Build response
	result := make([]LeaderboardEntry, 0, len(playerReps))
	for i, pr := range playerReps {
		if i >= 50 { // Top 50
			break
		}
...
// GetLeaderboardGuilds endpoint for guild rankings
func GetLeaderboardGuilds(c echo.Context) error {
	db := repository.GetDB()
	...
	// Calculate reputation for each guild (sum of members' reputation)
	type GuildReputation struct {
		Guild      domain.Guild
		Reputation int
		Members    int
	}
	guildReps := make([]GuildReputation, 0, len(guilds))

	for _, g := range guilds {
		reputation := 0
		members := 0

		var guildMembers []domain.GuildMember
		if err := db.Where("guild_id = ?", g.ID).Find(&guildMembers).Error; err == nil {
			members = len(guildMembers)
			for _, m := range guildMembers {
				var player domain.Player
				if err := db.First(&player, "id = ?", m.PlayerID).Error; err == nil {
					reputation += player.Reputation
				}
			}
		}

		guildReps = append(guildReps, GuildReputation{
			Guild:      g,
	...
	// Sort by reputation descending
	sort.Slice(guildReps, func(i, j int) bool {
		return guildReps[i].Reputation > guildReps[j].Reputation
	})

	// Build response
	result := make([]LeaderboardEntry, 0, len(guildReps))
	for i, gr := range guildReps {
		if i >= 50 { // Top 50
			break
		}
		result = append(result, LeaderboardEntry{
