...
func GetPvpTargets(c echo.Context) error {
	...
	for i, t := range targets {
		id := t.IslandID.String()
		if t.IsFleet && t.FleetID != nil {
			id = t.FleetID.String()
		}
		dtos[i] = PvpTargetDTO{
			ID:      id,
		...
		if t.FleetID != nil {
			dtos[i].FleetID = t.FleetID.String()
		}
	}
...
type AttackPvpResponse struct {
	CombatResult dto.CombatResultDTO `json:"combat_result"`
